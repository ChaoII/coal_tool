...
url = "http://127.0.0.1:8000/api/pointcloud/store_coal"
...
np.savetxt("store_coal_response_file/src_point.txt", np.array(src_point), "%.4f", " ")

for index, split_point in enumerate(split_points):
    np.savetxt(f"store_coal_response_file/split_point_{index}.txt", np.array(split_point), "%.4f", " ")
